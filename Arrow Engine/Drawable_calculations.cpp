...
void AE::drawable_calculations::center_text(sf::Text& txt, sf::View& view) //centers the sprite to the window's view
{
	txt.setPosition(view.getCenter().x - (txt.getGlobalBounds().width / 2), view.getCenter().y - (txt.getGlobalBounds().height / 2));
}

void AE::drawable_calculations::center_sprite(sf::Sprite& spr, sf::View& view) //the tidiness of this code could be improved by using templates, this would mean there wouldnt have to be seperate functions for sf::Texts and sf::Sprites
{
	spr.setPosition(view.getCenter().x - (spr.getTextureRect().width / 2), view.getCenter().y - (spr.getTextureRect().height / 2));
}

void AE::drawable_calculations::center_n_offset_text(sf::Text& txt, int x, int y, sf::View& view) //centres and applies an offset to a sf::Sprite or sf::Text
{
	txt.setPosition(view.getCenter().x - (txt.getGlobalBounds().width / 2) + x, view.getCenter().y - (txt.getGlobalBounds().height / 2) + y);
}

void AE::drawable_calculations::center_n_offset_sprite(sf::Sprite& spr, int x, int y, sf::View& view)
{
	spr.setPosition(view.getCenter().x - (spr.getTextureRect().width / 2) + x, view.getCenter().y - (spr.getTextureRect().height / 2) + y);
}

int AE::drawable_calculations::chk_L_click_spr(sf::RenderWindow &win, sf::RenderTexture& rt, sf::Sprite& spr, sf::Event &event) //checks if left click is pressed and if the mouse is inbetween the globalbounds of the sprite/text, darkens the sprite when the mouse is hovering over it
{
...
bool AE::drawable_calculations::chk_L_click_txt(sf::RenderWindow& win, sf::RenderTexture &rt, sf::Text& txt, sf::Event &event)
{
...
bool AE::drawable_calculations::load_n_check_spr(sf::Texture & texture, sf::Sprite& spr, std::string path) //loads a texture and sets a sprite the given texture, also handles errors if the sprite cannot be loaded
{
...
std::string AE::drawable_calculations::file_dialogue()
{
...
bool AE::drawable_calculations::chk_4_digit(std::string str)
{
