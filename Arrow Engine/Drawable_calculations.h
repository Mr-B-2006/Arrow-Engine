...
namespace AE
{
	...
	namespace drawable_calculations //this namespace is intended to be used to easily use common functions that will be required by the program, such as centering sprites or opening file dialogues
	{
		...
		inline bool load_n_chk_texture(sf::Texture& texture)
		{
			return true;
		}
		...
		template<class T, class U> //consider renaming to parent child and template types to P and C respectively
		...
		inline sf::Vector2f vectorf_components_to_int(sf::Vector2f vec) //converts the components of a sf::Vector2f to integers, useful for fixing issues with blurry text
		{
			sf::Vector2f conv_vec = sf::Vector2f((int)vec.x, (int)vec.y);
			return conv_vec;
		}
		...
		//we may be able to get away with using getGlobalBounds() ifwe get rid of the template type on visual studio (see center_two_transformables)
		inline sf::Vector2f get_spr_topright(sf::Sprite &spr) //the getGlobalBounds() function is not inherited from sf::Sprite's and sf::Text's parent (sf::Transformable), so we cant template this function :(
		{ 
			return sf::Vector2f(spr.getPosition().x + spr.getGlobalBounds().width, spr.getPosition().y);
		}

		inline sf::Vector2f get_txt_topright(sf::Text &txt)
		{
			return sf::Vector2f(txt.getPosition().x + txt.getGlobalBounds().width, txt.getPosition().y);
		}

		template <class T>
		inline sf::Vector2f get_shape_topright(T& shape) //we can template sf::Shape's getGlobalBounds() function for rectangle and circle shaped tho :D
		{
		...
		inline sf::Vector2f connect_sprite_to_shape(C &child, P &parent) //this function "connects" one transformable to another (places in parent's position + width + child's width)
		{
			return get_shape_topright(parent);// +sf::Vector2f(child.getGlobalBounds().width, 0);
		}

		inline void normalizeTextOrigin(sf::Text& text) {
			...
			// Get font metrics
			float ascent = font->getLineSpacing(characterSize);   // Height from baseline to top
			float descent = font->getGlyph('g', characterSize, false).bounds.top;
			float lineSpacing = font->getLineSpacing(characterSize);

			// Calculate bounding box for all characters
			sf::FloatRect bounds = text.getLocalBounds();

			// Set origin to the vertical center of the text (ascent - descent + line height adjustment)
			float verticalOffset = (ascent - descent) / 2.0f; // Adjust based on font line spacing
			text.setOrigin(bounds.left + bounds.width / 2.0f, bounds.top + verticalOffset);
		}
